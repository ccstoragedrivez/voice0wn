...
bot.on('voiceStateUpdate', async (oldState, state) => {
    ...
    try {
        ...
        return db.ref(state.guild.id).once('value').then(function (snapshot) {
            ...
            if (Date.now() - lastJoinTime.get(voiceChannel.id) < 120 * 60 * 1000) return lastJoinTime.set(voiceChannel.id, Date.now()); //return if last join is >30m ago + update last join
